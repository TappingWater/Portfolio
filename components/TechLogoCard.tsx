...
const TechLogoCard: React.FC<LogoCardProps> = ({ label, icons, styles }) => {
	...
	const beanstalkLink = {
		icon: "/cloud/beanstalk.svg",
		name: "AWS Elastic Beanstalk",
	};
	const gatewayLink = {
		icon: "/cloud/gateway.svg",
		name: "AWS API Gateway",
	};
	const amplifyLink = {
		icon: "/cloud/amplify.svg",
		name: "AWS Amplify",
	};
	...
	const linkMap: Map<string, imageLink> = new Map([
		...
		["echo", echoLink],
		["gpt", gptLink],
		["beanstalk", beanstalkLink],
		["amplify", amplifyLink],
		["apigateway", gatewayLink],
	]);

	let style =
		"bg-white text-black p-3 rounded-lg max-w-[70%] m-auto mt-[10px] mb-[10px]";
	if (styles) {
		style = styles;
	}
	...
	return (
		<div className={style}>
			{label && (
				<p className='text-center mb-2 mt-2 font-semibold'>{label}</p>
			)}
			<div className='flex flex-row space-x-8 justify-around flex-wrap gap-y-4'>
				{icons
					.filter((icon) => linkMap.has(icon))
					.map((icon) => (
